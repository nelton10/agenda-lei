...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubMaterials = onSnapshot(publicDataRef('materials'), (snapshot) => {
      ...
      // Carregar padrões se a base de dados estiver vazia
      if (data.length === 0) {
  ...
  const addMaterial = async (e) => {
    ...
    } catch (e) { notify("Erro ao cadastrar.", "error"); }
  };

  const deleteMaterial = async (id) => {
    if (id === 'lab') return notify("O Laboratório é fixo.", "error");
    try {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#F1F5F9] flex items-center justify-center p-4 font-sans text-slate-900">
        <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-500 border border-slate-200">
          ...
          <form onSubmit={handleLogin} className="p-10 space-y-7 text-slate-900">
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input required className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-blue-500 focus:bg-white outline-none transition-all font-bold text-slate-800 uppercase" placeholder="DIGITE SEU NOME" value={loginForm.username} onChange={(e) => setLoginForm({...loginForm, username: e.target.value})} />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input required type="password" className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-blue-500 focus:bg-white outline-none transition-all font-bold text-slate-800" placeholder="••••••••" value={loginForm.password} onChange={(e) => setLoginForm({...loginForm, password: e.target.value})} />
              </div>
            </div>
            ...
            <button type="submit" className="w-full py-5 bg-blue-600 text-white rounded-[1.8rem] font-black hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 flex items-center justify-center gap-2 tracking-widest uppercase text-sm border-b-4 border-blue-800 active:border-b-0 active:translate-y-1">Entrar no Sistema</button>
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-[#F8FAFC] overflow-hidden font-sans text-slate-900 print:bg-white">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col hidden lg:flex shrink-0 shadow-sm z-20 print:hidden text-slate-900">
        <div className="p-8 flex flex-col items-center border-b border-slate-50 bg-slate-50/30">
          ...
          <div className="text-center">
            <h1 className="font-black text-slate-800 leading-tight text-sm tracking-tighter uppercase">Anísio Teixeira</h1>
      ...
      <main className="flex-1 flex flex-col overflow-hidden pb-20 lg:pb-0 print:overflow-visible print:pb-0">
          ...
          <div className="flex items-center justify-between w-full lg:w-auto">
             ...
             {activeTab === 'agenda' && (
               <div className="relative group flex-1 lg:flex-none mx-4 lg:mx-0">
                  ...
                  <input type="text" placeholder="Filtrar agenda..." className="pl-11 pr-4 py-3 bg-slate-50 border-2 border-transparent rounded-2xl focus:border-blue-500 outline-none w-full lg:w-80 transition-all font-bold text-slate-800 text-xs shadow-inner" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
               </div>
             )}
          </div>
        ...
        <div className="flex-1 overflow-auto p-3 lg:p-8 animate-in fade-in print:p-0">
          {activeTab === 'agenda' && (
            <div className="space-y-6 print:space-y-0 text-slate-900">
              <div className="lg:hidden flex overflow-x-auto gap-3 pb-2 scrollbar-hide print:hidden">
                      ...
                      <tr key={horario.id} className={horario.type === 'pausa' ? 'bg-slate-50/40 print:bg-slate-100' : ''}>
                        <td className="w-16 lg:w-28 p-4 lg:p-8 border-r border-slate-100 text-center align-middle shrink-0">
                          <div className="flex flex-col items-center text-slate-900">
                            ...
                            {horario.title && <span className="text-[7px] font-black text-blue-500 uppercase mt-2 tracking-widest bg-blue-50 px-1.5 py-0.5 rounded-md whitespace-nowrap">{horario.title}</span>}
                          </div>
                        </td>
                        {weekDatesMemo.map((item, index) => {
                          ...
                          return (
                            <td key={`${item.date}-${horario.id}`} className={`p-2 lg:p-3.5 border-r border-slate-50 last:border-r-0 group relative min-h-[140px] align-top transition-colors hover:bg-blue-50/10 ${!isVisible ? 'hidden lg:table-cell' : ''}`}>
                              <div className="space-y-3">
                                {dayBookings.map(b => (
                                  <div key={b.id} className={`p-4 rounded-[1.5rem] relative shadow-lg animate-in fade-in border ${b.isBlocked ? 'bg-slate-900 text-white border-slate-800' : 'bg-white text-slate-900 border-slate-200 shadow-slate-100'}`}>
                                    <div className="flex justify-between items-start mb-3 pr-2 min-w-0">
                                      <div className="min-w-0"><p className="text-[10px] lg:text-[11px] font-black truncate uppercase">{b.isBlocked ? "BLOQUEADO" : b.teacher}</p><p className={`text-[8px] font-black uppercase mt-1 tracking-widest ${b.isBlocked ? 'text-slate-500' : 'text-blue-500'}`}>{b.room}</p></div>
                                    ...
                                    {!b.isBlocked && (
                                      <div className="flex flex-wrap gap-1.5">
                                        {b.resources.map(resId => {
                                          ...
                                          return res ? (
                                            <div key={resId} className="flex items-center gap-1 bg-slate-50 border border-slate-100 px-2 py-0.5 rounded-xl">
                                    ...
                                    {(user.isAdmin || b.teacher === user.username) && (
                                      <button onClick={e => { e.stopPropagation(); deleteBooking(b.id); }} className="absolute -top-2 -right-2 bg-white text-slate-400 hover:text-red-500 shadow-lg border p-2 rounded-full z-20 print:hidden active:scale-90"><Trash2 size={14} /></button>
                                    )}
                                  </div>
                                ))}
                                {dayBookings.length === 0 && <div className="h-16 flex items-center justify-center border-2 border-dashed border-slate-100 rounded-[1.5rem] opacity-30 print:hidden text-slate-300 transition-colors hover:border-blue-200" onClick={() => handleOpenModal(item.date, horario.id)}><Plus size={16} /></div>}
                              </div>
                              <button onClick={() => handleOpenModal(item.date, horario.id)} className="absolute inset-0 hidden lg:flex items-center justify-center opacity-0 group-hover:opacity-100 bg-blue-50/5 print:hidden text-blue-600"><Plus size={24} /></button>
                            </td>
                          );
                        })}
                      </tr>
          ...
          {activeTab === 'materials' && (
             <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in text-slate-900">
               <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-xl space-y-8 text-slate-900">
                 <h2 className="text-2xl font-black text-slate-800 uppercase flex items-center gap-3"><ShieldCheck className="text-blue-600"/> Gestão de Inventário</h2>
                 <form onSubmit={addMaterial} className="grid grid-cols-1 md:grid-cols-5 gap-5 text-slate-900">
                    <div className="space-y-1 text-slate-900"><span className="text-[10px] font-black text-slate-400 uppercase">Nome</span><input className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold text-slate-900 outline-none border-2 border-transparent focus:border-blue-500" value={newMaterial.name} onChange={e => setNewMaterial({...newMaterial, name: e.target.value})} /></div>
                    <div className="space-y-1 text-slate-900"><span className="text-[10px] font-black text-slate-400 uppercase">Sigla</span><input className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold uppercase text-slate-900 outline-none border-2 border-transparent focus:border-blue-500" value={newMaterial.short} onChange={e => setNewMaterial({...newMaterial, short: e.target.value.toUpperCase()})} /></div>
                    <div className="space-y-1 text-slate-900"><span className="text-[10px] font-black text-slate-400 uppercase">Tipo</span><select className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold text-slate-900 outline-none border-2 border-transparent focus:border-blue-500" value={newMaterial.isShareable} onChange={e => setNewMaterial({...newMaterial, isShareable: e.target.value === "true"})}><option value="false">Unitário</option><option value="true">Livre</option></select></div>
                    <div className="space-y-1 text-slate-900"><span className="text-[10px] font-black text-slate-400 uppercase">Cor</span><select className="w-full px-5 py-4 bg-slate-50 rounded-2xl font-bold text-slate-900 outline-none border-2 border-transparent focus:border-blue-500" value={newMaterial.color} onChange={e => setNewMaterial({...newMaterial, color: e.target.value})}><option value="bg-blue-600">Azul</option><option value="bg-emerald-600">Verde</option><option value="bg-purple-600">Roxo</option><option value="bg-red-600">Vermelho</option></select></div>
      ...
      {modalOpen && activeCell && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-xl z-[500] flex items-end lg:items-center justify-center p-0 lg:p-4 animate-in fade-in print:hidden text-slate-900">
          <div className="bg-white rounded-t-[3.5rem] lg:rounded-[4rem] w-full max-w-lg shadow-2xl flex flex-col max-h-[95vh] border border-slate-100 overflow-hidden text-slate-900 shadow-blue-200">
            <div className="bg-blue-600 p-10 text-white relative shrink-0">
              ...
              <button onClick={() => setModalOpen(false)} className="absolute top-10 right-10 text-white/50 hover:text-white transition-all"><X size={28}/></button>
            </div>
            
            <form onSubmit={saveBooking} className="p-10 space-y-8 overflow-y-auto scrollbar-hide pb-24 bg-white text-slate-900 text-slate-900">
